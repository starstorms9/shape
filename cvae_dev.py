...
class CVAE(tf.keras.Model):
    def __init__(self, latent_dim, input_dim):
        ...
        self.inference_net.add(Conv3D( filters=8, kernel_size=3, strides=(4, 4, 4), activation='relu'))
          
        self.generative_net = tf.keras.Sequential()
        self.generative_net.add(InputLayer(input_shape=(4,4,4,4)))
        self.generative_net.add(Conv3DTranspose( filters=64, kernel_size=3, strides=(2, 2, 2), padding="SAME", activation='relu'))
        ...
        self.generative_net.add(Conv3DTranspose( filters=8,  kernel_size=3, strides=(2, 2, 2), padding="SAME", activation='relu'))
    ...
    def log_normal_pdf(self, sample, mean, logvar, raxis=[1,2,3,4]):
    ...
    def printIO(self) :
        ...
        print(self.inference_net.input_shape)
        print(self.inference_net.output_shape)
        print("\nGenerative Net Summary:")
        print(self.generative_net.input_shape)
        print(self.generative_net.output_shape)
